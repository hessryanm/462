...
if (isset($_REQUEST['_name']) && $_REQUEST['_name'] == "delivery_ready" && isset($_REQUEST['_domain']) && $_REQUEST['_domain'] == "rfq"){
	...
	mysql_query("INSERT INTO delivery (shop_delivery_id, pickup_time, delivery_time, shop_id) VALUES ('$delivery_id', '$pickup_time', '$delivery_time', '$shop_id')") or die("Can't insert delivery: ".mysql_error());
	...
	mysql_query("UPDATE delivery SET driver_chosen = '$driver_id' WHERE shop_delivery_id = '$delivery_id' AND shop_id = '$shop_id'") or die("Can't update delivery: ".mysql_error());
	
	$driver_query = mysql_query("SELECT esl FROM users WHERE id = '$driver_id' LIMIT 1") or die("can't get esl: ".mysql_error());
	...
	$delivery_query = mysql_query("SELECT pickup_time, driver_chosen FROM delivery WHERE shop_delivery_id = '$delivery_id' AND shop_id = '$shop_id' LIMIT 1") or die("can't get delivery: ".mysql_error());
	...
	$driver_query = mysql_query("SELECT ranking FROM users WHERE id = '$driver_id' LIMIT 1") or die("Can't get driver: ".mysql_error());
	$driver = mysql_fetch_row($driver_query);
	$rank = $driver[0];
	$rank += $rank_change;
	
	mysql_query("UPDATE users SET ranking = '$rank' WHERE id = '$driver_id'") or die("can't update ranking: ".mysql_error());
	...
	$delivery_query = mysql_query("SELECT id, delivery_time, driver_chosen FROM delivery WHERE shop_delivery_id = '$delivery_id' AND shop_id = '$shop_id' LIMIT 1") or die("can't get delivery: ".mysql_error());
	$delivery = mysql_fetch_array($delivery_query);
	...
	mysql_query("DELETE FROM delivery WHERE id = '$guild_delivery_id'") or die("Can't delete: ".mysql_error());
